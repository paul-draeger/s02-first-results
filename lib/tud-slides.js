...
window.TUD = (function () {
  ...
  var TEXTS = {
    en: {
      ...
      videoToggle: 'Pause / resume the video on this slide'
    },
    de: {
      ...
      videoToggle: 'Video der Folie anhalten und weiterlaufen lassen'
    }
  };
  ...
  /* Die Videos einer Folie laufen von selbst und ohne Bedienleiste: sie sind
     Abbildungen, die sich bewegen, und keine Aufnahmen, die jemand abspielt.
     Im Saal fehlt damit der Halt -- wer auf eine Stelle im Lauf zeigen will,
     wartet sonst, bis die Schleife wieder dort ist. "v" haelt darum die Videos
     der aktuellen Folie an und laesst sie wieder laufen.

     Die Taste war bei reveal die zweite fuer den Schwarzschirm; B und . sind
     es weiterhin, der Schirm bleibt also erreichbar. Eine eigene Belegung
     greift vor der eingebauten, und 86 steht in reveals Liste der Tasten, die
     auch bei schwarzem Schirm noch ankommen -- zum Anhalten schadet das
     nicht. */
  function slideVideos() {
    var slide = window.Reveal && Reveal.getCurrentSlide && Reveal.getCurrentSlide();
    return slide ? Array.prototype.slice.call(slide.querySelectorAll('video')) : [];
  }

  function toggleVideos() {
    var videos = slideVideos();
    /* Laeuft eines, halten alle an; sonst laufen alle wieder. So bleibt ein
       Paar nebeneinander im selben Zustand, auch wenn eines davon beim Druck
       gerade am Ende seiner Schleife stand. */
    var running = videos.some(function (video) { return !video.paused && !video.ended; });
    videos.forEach(function (video) {
      if (running) { video.pause(); return; }
      var started = video.play();
      /* Ein play() darf der Browser ablehnen -- etwa bei einem Video, das
         ueber data-src noch laedt. Der Vortrag soll daran nicht mit einem
         Fehler in der Konsole haengen. */
  ...
  function revealConfig() {
    ...
    var base = {
      ...
      slideNumber: false,
      /* Die Tastenbelegung gehoert in die Konfiguration und nicht hinterher an
         Reveal.addKeyBinding(): die gibt es vor initialize() noch nicht (vorge-
         merkt werden nur configure, on, off, registerPlugin), und hinterher
         bliebe das Fenster bis "ready" -- in dem ein Druck auf v noch reveals
         eingebauten Schwarzschirm holt. Aus der Konfiguration gilt sie, sobald
         reveal ueberhaupt auf Tasten hoert. Die Beschriftung fuer die Hilfe
         kommt weiter unten dazu, die Konfiguration kennt keine. */
      keyboard: { 86: toggleVideos },
      controls: true,
    ...
    if (options.reveal.keyboard) merged.keyboard = assign({}, base.keyboard, options.reveal.keyboard);
  ...
  function init(userOptions) {
    ...
    return Reveal.initialize(revealConfig()).then(function (deck) {
      // Was die Taste tut, steht in der Hilfe (?) neben den eingebauten.
      Reveal.registerKeyboardShortcut('V', say('videoToggle'));
      return deck;
    });
  }
