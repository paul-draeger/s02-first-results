 ...
 *   - \cite, as `[@key]` in prose or <cite data-key="key">, numbered in order of
 *     first appearance and resolved against shared/literatur.bib;
...
window.TUD = (function () {
  ...
  /* `[@key]`, `[@key1; @key2]` in a text node, and <cite data-key="a,b">. Both
     become a <span class="cite">, numbered on the second pass. */
  ...
  function numberCitations() {
    ...
    Array.prototype.forEach.call(elements, function (node) {
      var numbers = node.getAttribute('data-key').split(',').map(function (key) {
        key = key.trim();
        if (!key) return null;
        if (!SHARED.bib[key]) {
        ...
        return citationOrder.indexOf(key) + 1;
  ...
  function bibliographyPages() {
    ...
    var pages = [];
    for (var start = 0; start < citationOrder.length; start += perPage) {
      var slice = citationOrder.slice(start, start + perPage);
      pages.push('<ol class="bibliography" start="' + (start + 1) + '">' +
